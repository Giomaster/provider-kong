...
import (
	...
	"github.com/Giomaster/provider-kong/apis/v1alpha1"
	"github.com/Giomaster/provider-kong/config"
	"github.com/Giomaster/provider-kong/internal/clients"
	"github.com/Giomaster/provider-kong/internal/controller"
	"github.com/Giomaster/provider-kong/internal/features"
)

func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for Kong").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-kong"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-kong",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apis.AddToScheme(mgr.GetScheme()), "Cannot add Kong APIs to scheme")
	...
	kingpin.FatalIfError(controller.Setup(mgr, o), "Cannot setup Kong controllers")
